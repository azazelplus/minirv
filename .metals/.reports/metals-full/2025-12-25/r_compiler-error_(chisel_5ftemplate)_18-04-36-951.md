...
### scala.reflect.internal.FatalError: no context found for source-file://<WORKSPACE>/src/main/scala/MiniRV.scala,line-6,offset=216
...
// flush: 在IDU(if_id_reg中)的跳转指令时启用. 
  //  pc[输出接输入打一拍], if_id_reg[输出接输入打一拍], id_ex_reg[插入气泡.]. 后续寄存器不影响.
