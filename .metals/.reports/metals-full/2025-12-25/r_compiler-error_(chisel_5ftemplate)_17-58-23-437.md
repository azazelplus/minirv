...
### scala.reflect.internal.FatalError: no context found for source-file://<WORKSPACE>/src/main/scala/MiniRV.scala,line-6,offset=196
...
// stall: 在IDU(if_id_reg中)的use指@@令I检测到发生load-use冒险时启用. 
  // pc[输出接输入打一拍], if_id_reg[输出接输入打一拍], id_ex_reg[插入气泡.]. 后续寄存器不影响.

// stall: 发生load-use冒险时启用. pc[输出接输入打一拍], if_id_reg[输出接输入打一拍], id_ex_reg[插入气泡.]. 后续寄存器不影响.
