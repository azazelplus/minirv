...
### scala.reflect.internal.FatalError: no context found for source-file://<WORKSPACE>/src/main/scala/MiniRV.scala,line-13,offset=436
...
// stall: 在IDU(if_id_reg中)的use指令I检测到发生load-use冒险时启用. 
  //  pc[输出接输入打一拍], if_id_reg[输出接输入打一拍], id_ex_reg[输入气泡]. 后续寄存器不影响.

// flush: 在EXU(id_ex_reg中)的指令被发现是jump_en时启用. 这包括情况: 该指令是jal, jalr, 或满足branch_taken的B指令. 
  //  pc[接收jump_en和jump_addr, 强制跳转], if_id_reg[内容@@替换为0], id_ex_reg[插入气泡.]. 后续寄存器不影响.
